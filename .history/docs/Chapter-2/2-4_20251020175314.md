...
2. 第二种常见的等价形式是**贝尔曼期望方程** (Bellman expectation equation)：
   ...
   这是因为$\mathbb{E}\left[G_{t+1}\mid S_t=s]=\sum_a \pi(a\mid s) \sum_{a^\prime}p(s^\prime \mid s,a)v_\pi(s^\prime)=\mathbb{E}[{v_\pi(S_{t+1})\mid S_t=s}]$，将其代入$(2.4)$中即可得到贝尔曼期望方程。
